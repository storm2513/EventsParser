class MainController < ApplicationController
	def index
		@events = Eventsbase.last
		render :json => @events, :except => [:updated_at]
	end

	def update
		render html: "Обновляем события..."
	...
	def version
		@events = Eventsbase.last
		UpdateWorker.perform_async
		render :json => @events, :except => [:data, :updated_at]
	end

	def count
		@events = Eventsbase.last
		render :json => @events.data.count
	end
